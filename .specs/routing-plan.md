...
- `.specs/31-cardio-log-data-layer.md` → codex, worktree wt-31, depends on: none. DTOs (`CardioModels.kt`) + `CardioLogRepo` + `AppContainer` wiring. No UI, no nav.
- `.specs/32-cardio-log-entry-screen.md` → codex, worktree wt-32, depends on: 31 merged. The log-entry form + its ViewModel + registers `Routes.CARDIO_LOG` in `Nav.kt`/`MainActivity.kt` (not yet reachable from anywhere in the UI — that's spec 33).
- `.specs/33-today-cardio-rollup.md` → codex, worktree wt-33, depends on: 31 AND 32 merged. Today-screen "🏃 Cardio: N/2 this week" rollup line, tappable → navigates to `Routes.CARDIO_LOG`.
- `.specs/34-cardio-log-history-screen.md` → codex, worktree wt-34, depends on: 31 AND 32 merged. The history list screen + registers `Routes.CARDIO_HISTORY`; adds a "History" top-bar button to spec 32's `CardioLogScreen.kt`.
